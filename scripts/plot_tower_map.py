...
for index, row in towers.iterrows():
    ax.plot(row['lons'], row['lats'], markersize=13, marker="*", color="tab:orange", markeredgecolor="black", markeredgewidth=0.5, transform=ccrs.PlateCarree())
    ax.plot(row['lons'], row['lats'], markersize=13, marker="*", color="tab:orange", markeredgecolor="black", markeredgewidth=0.5, transform=ccrs.PlateCarree())
    ax.text(row['lons'], row['lats']+0.7, row['cities'], fontsize='small', horizontalalignment='center', transform=ccrs.PlateCarree()) # bbox=dict(facecolor='red', alpha=0.7), 
