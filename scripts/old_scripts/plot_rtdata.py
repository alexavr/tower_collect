...
for trow in trows:
    ...
    cur.execute('SELECT equipment,Hz FROM equipment WHERE tower_name=? AND show=1', (tower_name,))
    erows = cur.fetchall()
    for erow in erows:
        equipment = erow[0]
        frequency = float(erow[1])
        print("    Working on tower %s, equipment %s:" % (tower_name, equipment))

        # if equipment != "A12":
        #     continue

        buffer_file = Path(config['buffer_path'],'%s_%s_BUFFER.npz' % (tower_name, equipment))
        # print(buffer_file)
        ...
        try:
            ...
            data = tl.data.create_empty_pd(equipment)

        # print(data)
        cur.execute('SELECT name,short_name,long_name,units,missing_value FROM variables WHERE tower_name=? AND equipment=?', (tower_name,equipment))
        vrows = cur.fetchall()

        if equipment == 'MET':
            figname_data = f"{config['wwwpath']}/static/{tower_name}_{equipment}_data24hr.png"
            plot = tl.plot.web_meteo_multiplt(tower_name, equipment, data, window_min=window_min,
            ...
            for vrow in vrows:
                ...
                figname_data = "%s/static/%s_%s_%s_data24hr.png" % (config['wwwpath'], tower_name, equipment, var_name)
                # figname_stat = "%s/static/%s_%s_%s_stat24hr.png" % (config['wwwpath'], tower_name, equipment, var_name)
